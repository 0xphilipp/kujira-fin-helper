...
import { FinClient, msg, registry } from "kujira.js";
...
// import contractsJson from '../../kujira-config/contracts.json'
// import denomsJson from '../../kujira-config/denoms.json';
//
...
const kujira = {
  ...
  async orders(
    ...
    const msgs = orders.map((o) => {
      ...
      const denom = o.side === "Buy" ? denoms.quote : denoms.base;
      let price = decimal_delta
        ? (o.price /= 10 ** decimal_delta).toFixed(
            decimal_delta + decimal_places
          )
      ...
      return msg.wasm.msgExecuteContract(data);
    });
  ...
  async getBalances(
    wallet: Wallet,
    contracts: Contract[],
    getDecimalDelta: Function
  ): Promise<Balance[]> {
    return fetch(
      `https://lcd.kaiyo.kujira.setten.io/cosmos/bank/v1beta1/balances/${wallet.account.address}`
    )
      .then((res) => res.json())
      .then((res) => {
        return res.balances.map((coin: Coin) => ({
          amount: `${
            +coin.amount / 10 ** getDecimalDelta(contracts, coin.denom)
          }`,
  ...
  async ordersCancel(
    ...
    return finClient.retractOrders({
      orderIdxs: orders.map((o) => `${o.idx}`),
    });
  },
  async getOrders(
    wallet: Wallet,
    ...
    const responses = await finClient
      .ordersByUser({
        address: account.address,
        limit: MAXIMUM_CONTRACT_LIMIT,
        startAfter:
          orders.length > 0 ? orders[orders.length - 1].idx : undefined,
      })
